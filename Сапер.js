...
var mines = 100;
	...
	// кнопка отвечает за начало генерации/регенерации поля
...
for (let i = 0;i<hor;i++) {
	...
	//добавляет нули в текст клетки/дает класс closed
...
// покраска, убирание нулей
document.querySelectorAll(".cell").forEach(item =>{
	let curText = Number(item.innerText);
	curText === 0 ? item.style.cssText = "font-size:0px;" : curText === 1 ? item.style.cssText = "color:#1D1DF5;" : curText === 2 ? item.style.cssText = "color:#0BF300" : curText === 3 ? item.style.cssText = "color:red" : curText === 4 ? item.style.cssText = "color:#070483" : curText === 5 ? item.style.cssText = "color:#BB0909" : curText === 6 ? item.style.cssText = "color:#0EBBBD" : curText === 7 ? item.style.cssText = "color:#CB15E6" : item.style.cssText = "color:#4B3131"
})
}
	...
	// добавление флага
	document.querySelectorAll(".cell").forEach(item => {
		item.addEventListener("contextmenu",function(elem) {
			elem.preventDefault()

			if ((elem.toElement.style.cssText.indexOf("flag") === -1) && (elem.toElement.className.indexOf("opened") === -1)) {

					elem.toElement.style.backgroundImage = "url(flag.svg)"
				} else {
					elem.toElement.style.backgroundImage = ""
				}
		})
	})
